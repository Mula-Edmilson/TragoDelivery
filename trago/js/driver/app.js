// ==================================================
// APLICAÇÃO DO MOTORISTA UNIFICADA (DRIVER APP)
// ==================================================

// ==================================================
// MODULE: driverTracking.js
// ==================================================
const DriverTracking = {
  ...
  init: () => {
    ...
    DriverTracking.socket.on('disconnect', () => {
      ...
      DriverUI.updateOnlineStatus(false);
    });

    // Escutar novas entregas
  ...
  startTracking: () => {
    if (!navigator.geolocation) {
      DriverUI.showLocationPermissionError('Geolocalização não é suportada por este navegador.');
      return;
    }

    // Pedir permissão e obter a posição
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        DriverUI.hideLocationPermissionError();
        DriverTracking.sendLocation(pos);

        // Configurar envio periódico a cada 10 segundos
      ...
      (err) => {
        DriverUI.showLocationPermissionError('Por favor, ative a permissão de localização para partilhar a sua posição em tempo real.');
      },
      { enableHighAccuracy: true }
    );
  },
  ...
  sendLocation: (pos) => {
    ...
    DriverUI.updateLastLocationTime();
  },
...
const DriverApp = {
  ...
  init: () => {
    ...
    // Inicializar Rastreamento e WebSockets
    DriverTracking.init();
  ...
  selectOrder: async (orderId) => {
    try {
      ...
      // Atualizar interface visual da lista para refletir a seleção
      ...
      if (order.status === 'atribuido' || order.status === 'pendente') {
        ...
        buttonsHtml = `
          <button onclick="DriverApp.updateOrderStatus('delivery-start')" class="w-full py-3 bg-purple-600 hover:bg-purple-70 text-white font-bold rounded shadow transition-all flex items-center justify-center gap-2 text-sm">
      ...
      // Se em mobile, fazer scroll suave para os detalhes
